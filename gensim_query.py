...
def validateopts(opts):
	...
	for option, value in opts:
		if option  in ["-h", "--help"]:
			...
			print "usage -u"
	return indexpath, corpuspath, sentence, numbest, threshold

def tips():
	"""Display the usage tips"""
	print "Please use: "+sys.argv[0]+" [options]"
	print "usage:%s --indexpath=value --corpuspath=value --sentence=value --numbest=value --threshold=value"
	print "usage:%s -i value -c value -s value -n value -t threshold"
	sys.exit(2)

def main():
	global rundate
	try:
		opts,args = getopt.getopt(sys.argv[1:],"hi:c:s:n:t:u",["indexpath=","corpuspath=","sentence=","numbest=","threshold=","help"])
	except getopt.GetoptError:
		tips()
	if len(opts) >= 3:
		indexpath, corpuspath, sentence, numbest, threshold = validateopts(opts)
	else:
	...
	gensimquery(indexpath, corpuspath, sentence, numbest, threshold)
